...
namespace AC
{
	...
	public class Char : MonoBehaviour, ITranslatable
	{
		...
		/** A factor to multiply the effects of IK head-turning by. Normally this should be left as 1, but may need reducing if the neck is made of multiple bones. */
		...
		private void OnAnimatorIK (int layerIndex)
		{
			if (ikHeadTurning)
			{
				...
				_animator.SetLookAtWeight (headTurnWeight * headIKTurnFactor);
			}
		}
		...
		private void PausePath (float pauseTime, ActionListAsset pauseAsset, int parameterID)
		{
			...
			if (pauseAsset.useParameters && parameterID >= 0 && pauseAsset.parameters.Count > parameterID)
			{
				...
				{
					ACDebug.LogWarning (this.gameObject.name + " requires a ConstantID script component!", gameObject);
				}
		...
		{
			if (Mathf.Approximately (targetHeadAngles.x, 0f) && KickStarter.stateHandler != null && KickStarter.stateHandler.IsInGameplay ())
			{
