	...
	public class SpeechManager : ScriptableObject
	{
				...
				if (translatable.CanTranslate (i))
				{
					...
					if (onlySeekNew != translatable.HasExistingTranslation (i))
					{
						if (onlySeekNew)
						{
							...
							SpeechLine newLine = new SpeechLine (GetEmptyID (), _scene, translatable.GetOwner (), textToTranslate, numTranslations, translatable.GetTranslationType (i), translatable.OwnerIsPlayer ());
						...
						else
						{
							...
							SpeechLine existingLine = new SpeechLine (translatable.GetTranslationID (i), _scene, translatable.GetOwner (), textToTranslate, numTranslations, translatable.GetTranslationType (i), translatable.OwnerIsPlayer ());
		...
		private void GetLinesInScene (string sceneFile, bool onlySeekNew)
		{
			...
			for (int i=0; i<sceneObjects.Length; i++)
			{
				...
				{
					ExtractTranslatable (currentComponent, onlySeekNew, true, true);
				}
			}
